...
We use an iteration over conventionalcommits.org.
...
- `BREAKING CHANGE`s are added only for self-hosting users to the commit body.
- `Refs:` (optional) either in the first line (to render as `<a href>` in GitHub log), or as last line.
    - Your issue tracker may parse `magic words` to auto-link commits - `Refs: {id}`, `Closes {id}, {id}, and {id}`.

Conventions:
- Capitalize the domain language: Django models, React components, etc - to visually distinct them.
- Django sub-types as `Post(type=Review)` are shortened to `Review`.
- React components wrap in brackets as `<LayoutSidebar>`.
...
Usually don't require a code review:
- `UI` - UI-only changes
- `format` - code style or formatting
- `docs`

Pick the highest in order `Type` when unsure - they're sorted by importance.
...
- `Algol` - <Term id="algolia"/>'s integration
...
- `build(Render)` - render.yaml
- `build(up)[<uv|pnpm|Mise>]` - pyproject.toml, package.json, lock files, or related changes
- `build(uv`/`pnpm`) - FE/BE dependencies
...
- `refac-pending` - for <LinkInt path="/development/reference/refactor-pending">refactor-pending.mdx file</LinkInt>
...
Added at the end of a commit first line. See <LinkInt path="/development/intro/code-style" hash="tags">Tags</LinkInt>.
