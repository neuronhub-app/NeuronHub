...
Focus on the last unchecked Task below.
...
- [x] Render
    - [x] create webhook view at `/jobs/send-emails/<secret>/` + URL route + `DJANGO_CRON_WEBHOOK_SECRET` setting
    - [x] add Algolia env vars to `worker` (add to `stage-backend` group)

## Relevant-Files

- render.yaml - ALGOLIA vars moved to `stage-backend` group; cron service added
- server/neuronhub/apps/jobs/views.py - webhook endpoint for cron
- server/neuronhub/urls.py - `/jobs/send-emails/<secret>` route
- server/neuronhub/settings.py - `DJANGO_CRON_WEBHOOK_SECRET`
- server/neuronhub/apps/jobs/tasks.py - fixed `MonitorConfig` import
- server/neuronhub/apps/jobs/views__test.py - webhook tests

## Decision-Log

- Fix: `MonitorConfig` runtime crash (sentry 2.53 TYPE_CHECKING-only)
    - pass plain dict to `monitor(monitor_config={})`
- Fix: ALGOLIA vars -> `stage-backend` group (worker needs them)
- Fix: added `views__test.py` — valid/invalid secret tests pass
