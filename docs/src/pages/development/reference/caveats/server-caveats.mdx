...
In custom fields of `@strawberry.type` or `@strawberry_django.type` - its `self` is never the actual class, but:
...
### Django Debug Toolbar "store" has no "set" method

`StoredDebugToolbar` v6.0.0 `init_store` method doesn't work, to fix it in `debug_toolbar/toolbar.py` change:
<CodeBlockText lang="python">{`
    def init_store(self):
