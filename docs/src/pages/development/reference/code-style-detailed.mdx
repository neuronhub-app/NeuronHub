---
order: 20
description: Detailed code style conventions, complementing the code-style intro.
---

import { CodeBlockText, LinkInt } from "@/components"
...
Hence a comment is not only the future tech debt - it is also the highest-level information noise, that damages code comprehension by its mere existence.

### Group related variables by domain in an object

<CodeBlockText lang="typescript">{`
  // Bad
...
If a file exports a single function - name it identically.


#### Existing Conventions

- functions using `get_or_create` -> `sync`
- pre-parsed -> `*_raw` postfix
- post-parsed -> `*_parsed`
- site-specific classes/functions:
    - NHA -> `Neuronhub` prefix
    - PG -> `Probablygood`
...
For multiple params -> prefer named over positional.

- `opts` for optional. Especially if it's the only param.
- `args` for required. Ok to merge `opts` into it for simplicity.
- `props` for a React component or hook.

### TODO comments

See <LinkInt path="/development/reference/todos">todos</LinkInt>.

Python
----------------------------------------

Use the modern Python3.14 syntax, including:
- ternary operator
- match/case
- `Enum`s
