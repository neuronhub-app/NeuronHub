...
order: 1
---
import { LinkInt, Mermaid } from "@/components"
...
- <LinkInt path="/development/reference/llm-spec-logs">LLM spec logs</LinkInt> - historical ticket-prompts LLM received and their Git history - ie it's complementary to the Git log. Named as `{id}-{type}-{name}.mdx` - `#{id}` is from the Git logs.
