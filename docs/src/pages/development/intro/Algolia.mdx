...
We serialize models on BE using GraphQL queries (eg `PostsByIds`, `JobsByIds`) in `_get_graphql_field`. This keeps the single-place GraphQL [cache reset](/development/intro/frontend/graphql#cache-reset): on list pages, Algolia hits are enriched via <Src to="useAlgoliaEnrichmentByGraphql.ts" /> so `mutateAndRefetchMountedQueries` auto-refetches on mutations.
...
Sub-sites (eg `sites/pg/`) reuse these components without changes - see [Sub-sites](/development/intro/frontend/sub-sites-with-vite_site).
...
For details see the [FE docs re prerender](/development/intro/frontend/prerender-and-prefetch).
