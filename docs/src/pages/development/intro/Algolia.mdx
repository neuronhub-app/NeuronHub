...
- `posts` - <Src to="posts/index.py" />
- `jobs` - <Src to="jobs/index.py" />
- `profiles` - <Src to="profiles/index.py" />

FE index names are in <Src to="useAlgoliaSearchClient.ts" />.

### 1:1 Algolia ↔ GraphQL compatibility

We serialize models on BE using GraphQL queries (eg `PostsByIds`, `JobsByIds`) in `_get_graphql_field`. This keeps the single-place GraphQL <LinkInt path="/development/intro/client/graphql" hash="reset-the-cache-on-every-mutation">cache reset</LinkInt>: on list pages, Algolia hits are enriched via <Src to="useAlgoliaEnrichmentByGraphql.ts" /> so `mutateAndRefetchMountedQueries` auto-refetches on mutations.
...
Uses `custom_objectID = "slug"` as `Job.id` changes on <Src to="jobs/services/publish_job_versions.py" />. Others use the default `id`.

### Bulk updates

Algolia provides 3 undocumented methods (used in `publish_job_versions.py`):
- `adapter.partial_update_objects`
- `adapter.save_objects`
...
Sub-sites (eg `sites/pg/`) reuse these components without changes - see <LinkInt path="/development/intro/client/sub-sites-with-vite_site">Sub-sites</LinkInt>.

## JobsLandingPage(Model) → Algolia state filter

The filters are converted to `IndexUiState` at <Src to="landingPageToAlgoliaState.ts" />.

For details see the <LinkInt path="/development/intro/client/prerender-and-prefetch">FE docs re prerender</LinkInt>.
