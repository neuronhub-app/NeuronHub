...
Instead of writing types by hand, you must use `gql-tada.ResultOf` or `FragmentOf`, eg:
...
`function isPost(post: Post | unknown): post is Post { return post.__typename === "PostType" }`.
...
You must use <Src to="mutateAndRefetchMountedQueries.tsx" /> function instead of `client.mutate`, to mitigate Apollo's dysfunctional caching. It also has `mutateDeleteAndResetStore()`, as `client.refetchQueries({ include: "all" })` does not refetch all queries.
...
Always use <Src to="useApolloQuery.ts" /> instead of the Apollo's `useQuery`.
