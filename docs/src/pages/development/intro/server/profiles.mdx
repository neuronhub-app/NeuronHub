...
import { CodeBlockText, LinkInt, Src } from "@/components"
...
For the UML overview of `profiles.models` see <LinkInt path="/development">arch/server</LinkInt>
...
- profile_for_llm_md: auto-serialized on save() via serialize_to_md(); user can overwrite it
...
- match_processed_at
- history
- review by User: match_score (0-100), match_review
...
- To avoid context rot, splits unscored Profiles into batches (10 by default).
...
Mirrors <Src to="posts/services/filter_posts_by_user.py" />. Algolia index uses `Profile.get_visible_to()` for token-restricted search (same as Posts: per-User).

### Testing

See <LinkInt path="/development/intro/server/pytest" hash="slow-tests">LLM API tests</LinkInt>.
