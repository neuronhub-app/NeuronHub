...
It's ok to create low-quality temporary pytest or Playwright cases for TDD - make sure to remove them afterwards.
...
- `apps/tests/services/db_stubs_repopulate.py` - seeds dev DB in `mise db_stubs_repopulate`.
    - `{app}/tests/db_stubs.py` - ~4 apps imported.
        - Each is 500+ LOC - mostly stubs for TextFields.
