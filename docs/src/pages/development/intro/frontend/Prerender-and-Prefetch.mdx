...
1. `mise client:prefetch-from-server` — runs <Src to="runPrefetch.ts" />. POSTs the `JobsLandingPagesForPrefetch` query (def for whitelist via <Src to="prefetch/JobsLandingPage.ts" />) and writes `client/graphql/prefetch/JobsLandingPages.json` (gitignored).
...
<Src to="useHeadMeta.tsx" /> — keyed by `location.pathname`. Render-time writes (not `useEffect`) so prerender emits the override. Root renders `<headMeta.Hoisted />`.
