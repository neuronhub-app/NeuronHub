...
- `.screenshot(name: str, { fullPage = false, maxH = 2000 })`: saves to `e2e/screenshots/`.
	- Note: it doesn't clean up old images.
...
- All data (except default user) is reset & reindex by Algolia by `reset_db_and_gen` GraphQL mutation before each test.
