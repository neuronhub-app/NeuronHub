...
class TestJobAlertSubscribe(NeuronTestCase):
    ...
    async def test_subscribe_with_utm_params(self):
        res = await self.graphql_query(
            """
            mutation {
                job_alert_subscribe(
        ...
        assert alert.utm_medium == "email"
        assert alert.utm_campaign == "jobs_weekly"
        assert alert.utm_content == ""
