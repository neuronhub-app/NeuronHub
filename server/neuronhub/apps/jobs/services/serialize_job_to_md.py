...
async def serialize_job_to_md(job: Job, is_show_org_desc: bool | None = True) -> str:
    lines = [
    ...
    lines.append("")
    lines.append("#### Tags")
    await _append_tags(lines, label="Areas", manager=job.tags_area)
    await _append_tags(lines, label="Skills", manager=job.tags_skill)
    ...
    if job.org.description and is_show_org_desc:
        lines.append("")
        lines.append("### Org")
