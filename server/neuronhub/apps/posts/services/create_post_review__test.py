...
class CreatePostReviewServiceTest(NeuronTestCase):
    ...
    async def test_creates_review_with_existing_tool(self):
        tool = await self.gen.posts.create(
            self.gen.posts.Params(type=Post.Type.Tool, title="React")
        )
    ...
    async def test_creates_tags_with_votes(self):
        data = PostTypeInput(
            ...
            tags=[
                PostTagTypeInput(
                    name="Python", is_vote_positive=True, comment="Great ecosystem"
                ),
        ...
        tags = await sync_to_async(
            lambda: list(review.tags.select_related("tag_parent").order_by("name"))
        )()
        assert len(tags) == 2

        python_vote = await PostTagVote.objects.aget(
            post=tool, tag__name="Python", author=self.user
        )
    ...
    async def test_handles_hierarchical_tags(self):
        data = PostTypeInput(
            ...
            tags=[PostTagTypeInput(name="Infrastructure / Container", is_vote_positive=True)],
        )
        ...
        data2 = PostTypeInput(
            ...
            tags=[
                PostTagTypeInput(
                    name="DevOps / Container / Orchestration", is_vote_positive=True
                )
            ],
        )
    ...
    async def test_updates_existing_tag_votes(self):
        tool = await self.gen.posts.create(
            self.gen.posts.Params(type=Post.Type.Tool, title="PostgreSQL")
        )
        python_tag = await PostTag.objects.acreate(name="Database", author=self.user)

        await PostTagVote.objects.acreate(
            post=tool, tag=python_tag, author=self.user, is_vote_positive=True, comment="Initial"
        )
    ...
    async def test_requires_parent_tool(self):
        ...
        data = PostTypeInput(title="Review without parent", review_rating=50)
