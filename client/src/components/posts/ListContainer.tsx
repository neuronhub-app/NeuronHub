...
export function ListContainer(props: {
  title: string;
  items: Array<Post | PostReview>;
  urlNamespace: "reviews" | "posts";
  isLoading: boolean;
  error?: CombinedError;
  children?: ReactNode;
}) {
  const isPostReview = props.items[0]?.__typename === "PostReviewType";
            ...
            {post => (
              <HStack key={post.id} gap="gap.md" align="flex-start">
                <Stack>
                  {isPostReview ? (
                    <PostButtonsVote
                      post={post}
                      mutation={graphql(`
                        mutation vote_review($id: ID!, $isVotePositive: Boolean) {
                          vote_review(id: $id, is_vote_positive: $isVotePositive)
                        }
                      `)}
                    />
                  ) : (
                    <PostButtonsVote
                      post={post}
                      mutation={graphql(`
                        mutation vote_post($id: ID!, $isVotePositive: Boolean) {
                          vote_post(id: $id, is_vote_positive: $isVotePositive)
                        }
                      `)}
                    />
                  )}
                ...
                >
                  ...
                  {post.tool && <ToolTags tags={post.tool.tags} />}
                  ...
                  <HStack justify="space-between">
                    ...
                    <HStack gap="gap.md" fontSize="sm">
                      ...
                      {post.tool?.crunchbase_url && (
                        <Link
                          href={`https://${post.tool.crunchbase_url}`}
                          color="fg.subtle"
                          variant="underline"
                        >
                          <Icon>
                      ...
                      {post.tool?.github_url && (
                        <Link
                          href={`https://${post.tool.github_url}`}
                          color="fg.subtle"
                          variant="underline"
                        >
                          <Icon>
